...
class PowerLawCreep( BaseMaterial ):
  ...
  def __init__ ( self, props ):
    ...
    self.n = self.exponent_n
    self.m = self.exponent_m
    self.A = self.factor
  ...
  def getStress( self, kinematics):
    ...
    if smises > ( 1.0 + self.tolerance ):
      ...
      while (abs(rhs) > self.tolerance or k==0):
        ...
        # print("k = ", k, rhs, deqpl)
        rhs   = self.A*pow(smises-self.eg3*deqpl,self.n) * exp_time * dt - deqpl
        jac = -self.A * self.eg3 * self.n * pow(smises-self.eg3*deqpl, self.n-1) * exp_time * dt - 1.0
        deqpl = deqpl - rhs/(jac)
